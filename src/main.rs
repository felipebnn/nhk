...
fn read_loop(dev: &mut Device, tx: Sender<InputEvent>) {
    dev.grab(GrabMode::Grab).unwrap();

    let mut key_map = match env::var("DEV_NAME") {
        Ok(val) => match val.as_str() {
            "KB" => key_map_kb::get_key_map(),
            "MOUSE" => key_map_mouse::get_key_map(),

            _ => {
                println!("DEV_NAME is unknown!");
                HashMap::new()
            }
        }

        Err(_) => {
            println!("DEV_NAME not present on env variables!");
            HashMap::new()
        }
    };
...
fn main() {
    let file_name = env::var("DEV_PATH").unwrap();
    ...
    let read_loop_thread = thread::spawn(move || read_loop(&mut dev, tx));
    ...
    read_loop_thread.join().expect("panic!");
}
